...
namespace Kosmic.SourceGenerators
{
    ...
    public class ScriptPropertiesGenerator : ISourceGenerator
    {
        ...
        )
        {
            ...
            if (godotClassProperties.Length > 0 || godotClassFields.Length > 0)
            {
                ...
                if (!allPropertiesAreReadOnly)
                {
                    ...
                    source.Append("        return base.SetKosmicClassPropertyValue(name, value);\n");
                ...
                if (!allPropertiesAreWriteOnly)
                {
                    ...
                    source.Append("        return base.GetKosmicClassPropertyValue(name, out value);\n");

                    source.Append("    }\n");
                }
                // Generate GetKosmicPropertyList

                const string DictionaryType = "global::System.Collections.Generic.List<global::Kosmic.Bridge.PropertyInfo>";
        ...
        {
            source.Append("        properties.Add(new(type: (global::Kosmic.Variant.Type)")
        ...
        private static IEnumerable<PropertyInfo> DetermineGroupingPropertyInfo(ISymbol memberSymbol)
        {
            foreach (var attr in memberSymbol.GetAttributes())
            {
                ...
                {
                    KosmicClasses.ExportCategoryAttr => PropertyUsageFlags.Category,
                    KosmicClasses.ExportGroupAttr => PropertyUsageFlags.Group,
                    KosmicClasses.ExportSubgroupAttr => PropertyUsageFlags.Subgroup,
                    _ => null
                };
        ...
        )
        {
            var exportAttr = memberSymbol.GetAttributes()
                .FirstOrDefault(a => a.AttributeClass?.IsKosmicExportAttribute() ?? false);

            var exportToolButtonAttr = memberSymbol.GetAttributes()
                .FirstOrDefault(a => a.AttributeClass?.IsKosmicExportToolButtonAttribute() ?? false);
        ...
        )
        {
            ...
            if (variantType == VariantType.Object && type is INamedTypeSymbol memberNamedType)
            {
                ...
                if (memberNamedType.InheritsFrom("KosmicSharp", "Kosmic.Resource"))
                {
                ...
                if (memberNamedType.InheritsFrom("KosmicSharp", "Kosmic.Node"))
                {
            ...
            static string GetTypeName(INamedTypeSymbol memberSymbol)
            {
                ...
                return memberSymbol.GetKosmicScriptNativeClassName()!;
            }
