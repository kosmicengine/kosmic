...
namespace KosmicTools.Export
{
    public partial class ExportPlugin : EditorExportPlugin
    {
        ...
        {
            return File.Exists(KosmicSharpDirs.ProjectSlnPath);
        }
        ...
        public override Kosmic.Collections.Array<Kosmic.Collections.Dictionary> _GetExportOptions(EditorExportPlatform platform)
        {
            return new Kosmic.Collections.Array<Kosmic.Collections.Dictionary>()
            {
                new Kosmic.Collections.Dictionary()
                {
                    {
                        "option", new Kosmic.Collections.Dictionary()
                        {
        ...
        public override void _ExportFile(string path, string type, string[] features)
        {
            ...
            if (!ProjectContainsDotNet())
            {
                GetExportPlatform().AddMessage(EditorExportPlatform.ExportMessageType.Error, "Export .NET Project", $"This project contains C# files but no solution file was found at the following path: {KosmicSharpDirs.ProjectSlnPath}\n" +
        ...
        private void _ExportBeginImpl(string[] features, bool isDebug, string path, long flags)
        {
            ...
            foreach (PublishConfig config in targets)
            {
                ...
                foreach (string arch in config.Archs)
                {
                    ...
                    string projectDataDirName = $"data_{KosmicSharpDirs.CSharpProjectName}_{platform}_{arch}";
                    if (platform == OS.Platforms.MacOS)
                    {
                    ...
                    string assemblyPath = Path.Combine(publishOutputDir, $"{KosmicSharpDirs.ProjectAssemblyName}.dll");
                    string nativeAotPath = Path.Combine(publishOutputDir,
                        $"{KosmicSharpDirs.ProjectAssemblyName}.{soExt}");
                    ...
                    RecursePublishContents(publishOutputDir,
                        ...
                        filterFile: file =>
                        {
                            if (platform == OS.Platforms.iOS)
                            {
                                ...
                                return Path.GetFileName(file) != $"{KosmicSharpDirs.ProjectAssemblyName}.dylib";
                            }
                        ...
                        addEntry: (path, isFile) =>
                        {
                            ...
                            if (isFile)
                            {
                                if (embedBuildResults)
                                {
                                    ...
                                    AddFile($"res://.kosmic/mono/publish/{arch}/{filePath}", fileData, false);
                                }
                                else
                                {
                    ...
                    if (embedBuildResults)
                    {
                        ...
                        AddFile($"res://.kosmic/mono/publish/{arch}/.dotnet-publish-manifest", fileData, false);
                    }
                }
            }
            ...
            if (platform == OS.Platforms.iOS)
            {
                if (outputPaths.Count > 2)
                {
                    ...
                    string outputPath = Path.Combine(outputPaths[1], $"{KosmicSharpDirs.ProjectAssemblyName}.dylib");
                    string[] files = outputPaths
                        .Skip(1)
                        .Select(path => Path.Combine(path, $"{KosmicSharpDirs.ProjectAssemblyName}.dylib"))
                        .ToArray();
                ...
                string xcFrameworkPath = Path.Combine(KosmicSharpDirs.ProjectBaseOutputPath, publishConfig.BuildConfig, $"{KosmicSharpDirs.ProjectAssemblyName}_aot.xcframework");
