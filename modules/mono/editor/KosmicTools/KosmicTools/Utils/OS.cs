...
namespace KosmicTools.Utils
{
    ...
    public static class OS
    {
        ...
        {
            Internal.kosmic_icall_Utils_OS_GetPlatformName(out kosmic_string dest);
            using (dest)
            {
        ...
        private static string? PathWhichUnix(string name)
        {
            ...
            return searchDirs.Select(dir => Path.Combine(dir, name))
                .FirstOrDefault(path =>
                {
                    using kosmic_string pathIn = Marshaling.ConvertStringToNative(path);
                    return File.Exists(path) && Internal.kosmic_icall_Utils_OS_UnixFileHasExecutableAccess(pathIn);
                });
        }
