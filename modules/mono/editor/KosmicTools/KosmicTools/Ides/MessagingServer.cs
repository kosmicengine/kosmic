...
namespace KosmicTools.Ides
{
    public sealed class MessagingServer : IDisposable
    {
        ...
        public MessagingServer(string editorExecutablePath, string projectMetadataDir, ILogger logger)
        {
            ...
            _metaFilePath = Path.Combine(projectMetadataDir, KosmicIdeMetadata.DefaultFileName);
        ...
        private class ServerMessageHandler : IMessageHandler
        {
            private static void DispatchToMainThread(Action action)
            {
                ...
                Kosmic.Dispatcher.SynchronizationContext.Post(d, null);
            }
            ...
            private static Task<Response> HandleDebugPlay(DebugPlayRequest request)
            {
                DispatchToMainThread(() =>
                {
                    ...
                    KosmicSharpEditor.Instance.SkipBuildBeforePlaying = !(request.BuildBeforePlaying ?? true);
                    ...
                    Environment.SetEnvironmentVariable("KOSMIC_MONO_DEBUGGER_AGENT",
                    ...
                    Environment.SetEnvironmentVariable("KOSMIC_MONO_DEBUGGER_AGENT", "");
                    KosmicSharpEditor.Instance.SkipBuildBeforePlaying = false;
                });
            ...
            private static async Task<Response> HandleCodeCompletionRequest(CodeCompletionRequest request)
            {
                ...
                await Kosmic.Engine.GetMainLoop().ToSignal(Kosmic.Engine.GetMainLoop(), "process_frame");
