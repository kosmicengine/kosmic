...
{
    public sealed partial class KosmicIdeManager : Node, ISerializationListener
    {
        ...
        private MessagingServer GetRunningOrNewServer()
        {
            ...
            _messagingServer = new MessagingServer(OS.GetExecutablePath(),
                ProjectSettings.GlobalizePath(KosmicSharpDirs.ResMetadataDir), new KosmicLogger());
        ...
        public async Task<EditorPick?> LaunchIdeAsync(int millisecondsTimeout = 10000)
        {
            ...
            var editorId = editorSettings.GetSetting(KosmicSharpEditor.Settings.ExternalEditor).As<ExternalEditorId>();
        ...
        private void LaunchIde(ExternalEditorId editorId, string editorIdentity)
        {
            switch (editorId)
            {
                ...
                case ExternalEditorId.MonoDevelop:
                {
                    ...
                    try
                    {
                        var instance = GetMonoDevelopInstance(KosmicSharpDirs.ProjectSlnPath);
                    ...
                    catch (FileNotFoundException)
                    {
                        ...
                        KS.PushError($"Cannot find code editor: {editorName}");
                    }
        ...
        public readonly struct EditorPick
        {
            ...
            public bool IsAnyConnected() =>
                KosmicSharpEditor.Instance.KosmicIdeManager.GetRunningOrNewServer().IsAnyConnected(_identity);
                ...
                where TResponse : Response, new()
            {
                ...
                KosmicSharpEditor.Instance.KosmicIdeManager
        ...
        private class KosmicLogger : ILogger
        {
