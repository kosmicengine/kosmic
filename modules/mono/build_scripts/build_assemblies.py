...
def build_kosmic_api(msbuild_tool, module_dir, output_dir, push_nupkgs_local, precision, no_deprecated, werror):
    target_filenames = [
        "KosmicSharp.dll",
        "KosmicSharp.pdb",
        "KosmicSharp.xml",
        "KosmicSharpEditor.dll",
        "KosmicSharpEditor.pdb",
        "KosmicSharpEditor.xml",
        "KosmicPlugins.dll",
        "KosmicPlugins.pdb",
        "KosmicPlugins.runtimeconfig.json",
    ]

    for build_config in ["Debug", "Release"]:
        editor_api_dir = os.path.join(output_dir, "KosmicSharp", "Api", build_config)
        ...
        sln = os.path.join(module_dir, "glue/KosmicSharp/KosmicSharp.sln")
        ...
        core_src_dir = os.path.abspath(os.path.join(sln, os.pardir, "KosmicSharp", "bin", build_config))
        editor_src_dir = os.path.abspath(os.path.join(sln, os.pardir, "KosmicSharpEditor", "bin", build_config))
        plugins_src_dir = os.path.abspath(os.path.join(sln, os.pardir, "KosmicPlugins", "bin", build_config, "net8.0"))
...
def generate_sdk_package_versions():
    ...
    version_defines = (
        [
            f"KOSMIC{version.major}",
            f"KOSMIC{version.major}_{version.minor}",
            f"KOSMIC{version.major}_{version.minor}_{version.patch}",
        ]
        + [f"KOSMIC{v}_OR_GREATER" for v in range(4, version.major + 1)]
        + [f"KOSMIC{version.major}_{v}_OR_GREATER" for v in range(0, version.minor + 1)]
        + [f"KOSMIC{version.major}_{version.minor}_{v}_OR_GREATER" for v in range(0, version.patch + 1)]
    )
...
""".format(version_str, ";".join(version_defines))
    ...
    constants = """namespace Kosmic.SourceGenerators
{{
// TODO: This is currently disabled because of https://github.com/dotnet/roslyn/issues/52904
#pragma warning disable IDE0040 // Add accessibility modifiers.
    partial class Common
    {{
        public const string VersionDocsUrl = "https://docs.kosmicengine.org/en/{docs_branch}";
    }}
}}
""".format(**version_info)
...
def build_all(
    msbuild_tool, module_dir, output_dir, kosmic_platform, dev_debug, push_nupkgs_local, precision, no_deprecated, werror
):
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Build KosmicSharp without using deprecated features. This is required, if the engine was built with 'deprecated=no'.",
    )
    ...
    output_dir = os.path.abspath(args.kosmic_output_dir)
