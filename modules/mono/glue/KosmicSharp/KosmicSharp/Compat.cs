...
{
    /// <inheritdoc cref="SegmentIntersectsConvex(Vector3, Vector3, Kosmic.Collections.Array{Plane})"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public static Vector3[] SegmentIntersectsConvex(Vector3 from, Vector3 to, Kosmic.Collections.Array planes)
    {
        return SegmentIntersectsConvex(from, to, new Kosmic.Collections.Array<Plane>(planes));
    }
}
...
{
    /// <inheritdoc cref="AddSurface(Mesh.PrimitiveType, Kosmic.Collections.Array, Kosmic.Collections.Array{Kosmic.Collections.Array}, Kosmic.Collections.Dictionary, Material, string, ulong)"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public void AddSurface(Mesh.PrimitiveType primitive, Kosmic.Collections.Array arrays, Kosmic.Collections.Array<Kosmic.Collections.Array> blendShapes, Kosmic.Collections.Dictionary lods, Material material, string name, uint flags)
    {
...
{
    /// <inheritdoc cref="DrawListBegin(Rid, InitialAction, FinalAction, InitialAction, FinalAction, Color[], float, uint, Nullable{Rect2}, Kosmic.Collections.Array{Rid})"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public long DrawListBegin(Rid framebuffer, InitialAction initialColorAction, FinalAction finalColorAction, InitialAction initialDepthAction, FinalAction finalDepthAction, Color[] clearColorValues, float clearDepth, uint clearStencil, Nullable<Rect2> region, Kosmic.Collections.Array storageTextures)
    {
        return DrawListBegin(framebuffer, initialColorAction, finalColorAction, initialDepthAction, finalDepthAction, clearColorValues, clearDepth, clearStencil, region, new Kosmic.Collections.Array<Rid>(storageTextures));
    }
}
...
{
    /// <inheritdoc cref="AddTriangleFan(Vector3[], Vector2[], Color[], Vector2[], Vector3[], Kosmic.Collections.Array{Plane})"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public void AddTriangleFan(Vector3[] vertices, Vector2[] uvs, Color[] colors, Vector2[] uv2S, Vector3[] normals, Kosmic.Collections.Array tangents)
    {
        AddTriangleFan(vertices, uvs, colors, uv2S, normals, new Kosmic.Collections.Array<Plane>(tangents));
    }
