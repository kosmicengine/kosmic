...
namespace Kosmic.NativeInterop
{
    internal static class ExceptionUtils
    {
        ...
        private static void SendToScriptDebugger(Exception e)
        {
            ...
            using kosmic_string nFile = Marshaling.ConvertStringToNative(file);
            using kosmic_string nFunc = Marshaling.ConvertStringToNative(func);
            using kosmic_string nErrorMsg = Marshaling.ConvertStringToNative(errorMsg);
            using kosmic_string nExcMsg = Marshaling.ConvertStringToNative(excMsg.ToString());

            using DebuggingUtils.kosmic_stack_info_vector stackInfoVector = default;
            ...
            unsafe
            {
                ...
                {
                    DebuggingUtils.kosmic_stack_info* stackInfo = &stackInfoVector.Elements[i];
                ...
                NativeFuncs.kosmicsharp_internal_script_debugger_send_error(nFunc, nFile, line,
                    nErrorMsg, nExcMsg, kosmic_error_handler_type.ERR_HANDLER_ERROR, stackInfoVector);
            }
        }
        ...
        public static void LogException(Exception e)
        {
            try
            {
                if (NativeFuncs.kosmicsharp_internal_script_debugger_is_active().ToBool())
                {
        ...
        public static void LogUnhandledException(Exception e)
        {
            try
            {
                ...
                KS.PushError("Unhandled exception\n" + e);
            }
            catch (Exception unexpected)
            {
                OnExceptionLoggerException(unexpected, e);
        ...
        [Conditional("DEBUG")]
        public unsafe static void DebugCheckCallError(kosmic_string_name method, IntPtr instance, kosmic_variant** args, int argCount, kosmic_variant_call_error error)
        {
            if (error.Error != kosmic_variant_call_error_error.KOSMIC_CALL_ERROR_CALL_OK)
            {
                using kosmic_variant instanceVariant = VariantUtils.CreateFromKosmicObjectPtr(instance);
                ...
                KS.PushError(errorText);
            }
        }

        [Conditional("DEBUG")]
        public unsafe static void DebugCheckCallError(in kosmic_callable callable, kosmic_variant** args, int argCount, kosmic_variant_call_error error)
        {
            if (error.Error != kosmic_variant_call_error_error.KOSMIC_CALL_ERROR_CALL_OK)
            {
                using kosmic_variant callableVariant = VariantUtils.CreateFromCallableTakingOwnershipOfDisposableValue(callable);
        ...
        private unsafe static string GetCallErrorWhere(ref kosmic_variant_call_error error, kosmic_string_name method, kosmic_variant* instance, kosmic_variant** args, int argCount)
        {
            string? methodstr = null;
            string basestr = GetVariantTypeName(instance);

            if (method == KosmicObject.MethodName.Call || (basestr == "Kosmic.TreeItem" && method == TreeItem.MethodName.CallRecursive))
            {
                if (argCount >= 1)
                {
                    methodstr = VariantUtils.ConvertToString(*args[0]);
                    if (error.Error == kosmic_variant_call_error_error.KOSMIC_CALL_ERROR_CALL_ERROR_INVALID_ARGUMENT)
                    {
                        error.Argument += 1;
                    }
                }
        ...
        private unsafe static string GetCallErrorMessage(kosmic_variant_call_error error, string where, kosmic_variant** args)
        {
            switch (error.Error)
            {
                case kosmic_variant_call_error_error.KOSMIC_CALL_ERROR_CALL_ERROR_INVALID_ARGUMENT:
                {
                ...
                case kosmic_variant_call_error_error.KOSMIC_CALL_ERROR_CALL_ERROR_TOO_MANY_ARGUMENTS:
                case kosmic_variant_call_error_error.KOSMIC_CALL_ERROR_CALL_ERROR_TOO_FEW_ARGUMENTS:
                    return $"Invalid call to {where}. Expected {error.Expected} arguments.";
                case kosmic_variant_call_error_error.KOSMIC_CALL_ERROR_CALL_ERROR_INVALID_METHOD:
                    return $"Invalid call. Nonexistent {where}.";
                case kosmic_variant_call_error_error.KOSMIC_CALL_ERROR_CALL_ERROR_INSTANCE_IS_NULL:
                    return $"Attempt to call {where} on a null instance.";
                case kosmic_variant_call_error_error.KOSMIC_CALL_ERROR_CALL_ERROR_METHOD_NOT_CONST:
        ...
        private unsafe static string GetVariantTypeName(kosmic_variant* variant)
        {
            if (variant->Type == Variant.Type.Object)
            {
                KosmicObject obj = VariantUtils.ConvertToKosmicObject(*variant);
                if (obj == null)
                {
                    return "null instance";
                }
                else if (!KosmicObject.IsInstanceValid(obj))
                {
