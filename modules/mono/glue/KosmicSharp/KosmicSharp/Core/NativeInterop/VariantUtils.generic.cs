...
public partial class VariantUtils
{
    ...
    {
        internal delegate kosmic_variant ToVariantConverter(scoped in T from);
        internal delegate T FromVariantConverter(in kosmic_variant from);

        public static unsafe kosmic_variant ToVariant(scoped in T from) =>
             ToVariantCb != null ? ToVariantCb(from) : throw UnsupportedType<T>();

        public static unsafe T FromVariant(in kosmic_variant variant) =>
    ...
    public static kosmic_variant CreateFrom<[MustBeVariant] T>(scoped in T from)
    {
        ...
        if (typeof(T) == typeof(Kosmic.Collections.Dictionary))
            return CreateFromDictionary(UnsafeAs<Kosmic.Collections.Dictionary>(from));

        if (typeof(T) == typeof(Kosmic.Collections.Array))
            return CreateFromArray(UnsafeAs<Kosmic.Collections.Array>(from));

        if (typeof(T) == typeof(Variant))
            return NativeFuncs.kosmicsharp_variant_new_copy((kosmic_variant)UnsafeAs<Variant>(from).NativeVar);
        ...
        if (typeof(KosmicObject).IsAssignableFrom(typeof(T)))
            return CreateFromKosmicObject(UnsafeAs<KosmicObject>(from));
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static T ConvertTo<[MustBeVariant] T>(in kosmic_variant variant)
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        if (typeof(KosmicObject).IsAssignableFrom(typeof(T)))
            return (T)(object)ConvertToKosmicObject(variant);
