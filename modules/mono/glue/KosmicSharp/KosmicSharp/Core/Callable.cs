...
namespace Kosmic
{
    ...
    {
        private readonly KosmicObject _target;
        private readonly StringName _method;
        private readonly Delegate _delegate;
        private readonly unsafe delegate* managed<object, NativeVariantPtrArgs, out kosmic_variant, void> _trampoline;

        /// <summary>
        /// Object that contains the method.
        /// </summary>
        public KosmicObject Target => _target;
        ...
        /// </summary>
        public unsafe delegate* managed<object, NativeVariantPtrArgs, out kosmic_variant, void> Trampoline
            => _trampoline;
        ...
        public unsafe Callable(KosmicObject target, StringName method)
        {
            _target = target;
            _method = method;
            _delegate = null;
        ...
        {
            _target = @delegate?.Target as KosmicObject;
            _method = null;
            _delegate = @delegate;
        ...
        {
            using kosmic_callable callable = Marshaling.ConvertCallableToNative(this);

            int argc = args.Length;

            Span<kosmic_variant.movable> argsStoreSpan = argc <= VarArgsSpanThreshold ?
                stackalloc kosmic_variant.movable[VarArgsSpanThreshold] :
                new kosmic_variant.movable[argc];

            Span<IntPtr> argsSpan = argc <= VarArgsSpanThreshold ?
                stackalloc IntPtr[VarArgsSpanThreshold] :
                new IntPtr[argc];

            fixed (kosmic_variant* varargs = &MemoryMarshal.GetReference(argsStoreSpan).DangerousSelfRef)
            fixed (IntPtr* argsPtr = &MemoryMarshal.GetReference(argsSpan))
            {
                for (int i = 0; i < argc; i++)
                {
                    varargs[i] = (kosmic_variant)args[i].NativeVar;
                    argsPtr[i] = new IntPtr(&varargs[i]);
                }

                kosmic_variant ret = NativeFuncs.kosmicsharp_callable_call(callable,
                    (kosmic_variant**)argsPtr, argc, out kosmic_variant_call_error vcall_error);
                ExceptionUtils.DebugCheckCallError(callable, (kosmic_variant**)argsPtr, argc, vcall_error);
        ...
        public unsafe void CallDeferred(params Variant[] args)
        {
            ...
            fixed (IntPtr* argsPtr = &MemoryMarshal.GetReference(argsSpan))
            {
                ...
                NativeFuncs.kosmicsharp_callable_call_deferred(callable, (kosmic_variant**)argsPtr, argc);
            }
        }
        ...
        ///    <item>
        ///        <term>args</term>
        ///        <description>Array of <see cref="kosmic_variant"/> arguments.</description>
        ///    </item>
        ///    <item>
        ///        <term>ret</term>
        ///        <description>Return value of type <see cref="kosmic_variant"/>.</description>
        ///    </item>
        ///</list>
        /// <para>
        ...
        ///
        /// <code>
        ///     static void Trampoline(object delegateObj, NativeVariantPtrArgs args, out kosmic_variant ret)
        ///     {
        ///         if (args.Count != 1)
