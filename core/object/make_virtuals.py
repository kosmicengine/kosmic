proto = """#define KSVIRTUAL$VER($RET m_name $ARG)\\
	StringName _ksvirtual_##m_name##_sn = #m_name;\\
	mutable bool _ksvirtual_##m_name##_initialized = false;\\
	mutable void *_ksvirtual_##m_name = nullptr;\\
	_FORCE_INLINE_ bool _ksvirtual_##m_name##_call($CALLARGS) $CONST {\\
		ScriptInstance *_script_instance = ((Object *)(this))->get_script_instance();\\
		if (_script_instance) {\\
			Callable::CallError ce;\\
			$CALLSIARGS\\
			$CALLSIBEGIN_script_instance->callp(_ksvirtual_##m_name##_sn, $CALLSIARGPASS, ce);\\
			if (ce.error == Callable::CallError::CALL_OK) {\\
				$CALLSIRET\\
				return true;\\
			}\\
		}\\
		if (unlikely(_get_extension() && !_ksvirtual_##m_name##_initialized)) {\\
			_ksvirtual_##m_name = nullptr;\\
			if (_get_extension()->get_virtual_call_data && _get_extension()->call_virtual_with_data) {\\
				_ksvirtual_##m_name = _get_extension()->get_virtual_call_data(_get_extension()->class_userdata, &_ksvirtual_##m_name##_sn);\\
			} else if (_get_extension()->get_virtual) {\\
				_ksvirtual_##m_name = (void *)_get_extension()->get_virtual(_get_extension()->class_userdata, &_ksvirtual_##m_name##_sn);\\
			}\\
			KSVIRTUAL_TRACK(_ksvirtual_##m_name, _ksvirtual_##m_name##_initialized);\\
			_ksvirtual_##m_name##_initialized = true;\\
		}\\
		if (_ksvirtual_##m_name) {\\
			$CALLPTRARGS\\
			$CALLPTRRETDEF\\
			if (_get_extension()->get_virtual_call_data && _get_extension()->call_virtual_with_data) {\\
				_get_extension()->call_virtual_with_data(_get_extension_instance(), &_ksvirtual_##m_name##_sn, _ksvirtual_##m_name, $CALLPTRARGPASS, $CALLPTRRETPASS);\\
				$CALLPTRRET\\
			} else {\\
				((KSExtensionClassCallVirtual)_ksvirtual_##m_name)(_get_extension_instance(), $CALLPTRARGPASS, $CALLPTRRETPASS);\\
				$CALLPTRRET\\
			}\\
			return true;\\
		}\\
	...
	_FORCE_INLINE_ bool _ksvirtual_##m_name##_overridden() const {\\
		ScriptInstance *_script_instance = ((Object *)(this))->get_script_instance();\\
		if (_script_instance && _script_instance->has_method(_ksvirtual_##m_name##_sn)) {\\
			return true;\\
		}\\
		if (unlikely(_get_extension() && !_ksvirtual_##m_name##_initialized)) {\\
			_ksvirtual_##m_name = nullptr;\\
	...
	_FORCE_INLINE_ static MethodInfo _ksvirtual_##m_name##_get_method_info() {\\
...
def generate_version(argcount, const=False, returns=False, required=False):
    ...
    if argcount > 0:
        ...
        callptrargsptr = f"\t\t\tKSExtensionConstTypePtr argptrs[{argcount}] = {{ "
    callptrargs = ""
    ...
    if argcount:
        ...
        s = s.replace("$CALLPTRARGPASS", "reinterpret_cast<KSExtensionConstTypePtr *>(argptrs)")
    else:
...
#ifndef KSVIRTUAL_GEN_H
#define KSVIRTUAL_GEN_H
...
#ifdef TOOLS_ENABLED
#define KSVIRTUAL_TRACK(m_virtual, m_initialized)\\
