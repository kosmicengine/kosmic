...
for file in sys.argv[1:]:
    ...
    for idx, line in enumerate(lines):
        ...
        if header_start < 0:
            ...
            if sline.startswith("/**********"):  # Kosmic header starts this way.
                header_start = idx
            else:
                HEADER_CHECK_OFFSET = 0  # There is no Kosmic header.
                break
        else:
            if not sline.startswith("*") and not sline.startswith("/*"):  # Not in the Kosmic header anymore.
                HEADER_CHECK_OFFSET = idx + 1  # The include should be two lines below the Kosmic header.
                break
