...
if [ ! -e "version.py" ]; then
  echo "This script should be ran from the root folder of the Kosmic repository."
  exit 1
fi
...
while getopts "h?sv:g:" opt; do
  ...
  h|\?)
    ...
    echo
    echo "  -s script friendly file name (kosmic.tar.gz)"
    echo "  -v kosmic version for file name (e.g. 4.0-stable)"
...
if [ ! -z "$script_friendly_name" ]; then
  ...
  if [ ! -z "$kosmic_version" ]; then
    NAME=kosmic-$kosmic_version
  else
    NAME=kosmic-$HEAD
  fi
fi
...
CURDIR=$(pwd)
TMPDIR=$(mktemp -d -t kosmic-XXXXXX)
