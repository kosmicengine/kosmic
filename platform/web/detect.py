...
def get_opts():
    ...
    return [
        ...
        # Matches default values from before Emscripten 3.1.27. New defaults are too low for Kosmic.
        ...
        BoolVariable(
            "dlink_enabled", "Enable WebAssembly dynamic linking (KSExtension support). Produces bigger binaries", False
        ),
...
def configure(env: "SConsEnvironment"):
    ...
    if env["dlink_enabled"]:
        if env["proxy_to_pthread"]:
            print_warning("KSExtension support requires proxy_to_pthread=no, disabling proxy to pthread.")
            env["proxy_to_pthread"] = False
