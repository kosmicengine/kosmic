...
KOSMIC_DOCS_PATTERN = re.compile(r"^\$DOCS_URL/(.*)\.html(#.*)?$")
...
# `scripts/extract_classes.py` (kosmicengine/kosmic-editor-l10n repo) to include them in the source POT file.
BASE_STRINGS = [
    "All classes",
    "Globals",
    "Nodes",
...
# Checks if code samples have both VoyScript and C# variations.
# For simplicity we assume that a VoyScript example is always present, and ignore contexts
...
class ScriptLanguageParityCheck:
    ...
    def add_hit(self, class_name: str, context: DefinitionBase, error: str, state: State) -> None:
        if class_name in ["@VoyScript", "@GlobalScope"]:
...
def make_rst_class(class_def: ClassDef, state: State, dry_run: bool, output_dir: str) -> None:
    ...
    with open(
        ...
        source_github_url = f"https://github.com/kosmicengine/kosmic/tree/{git_branch}/{source_xml_path}"
        generator_github_url = f"https://github.com/kosmicengine/kosmic/tree/{git_branch}/doc/tools/make_rst.py"
...
def make_link(url: str, title: str) -> str:
    match = KOSMIC_DOCS_PATTERN.search(url)
    if match:
        groups = match.groups()
...
def make_rst_index(grouped_classes: Dict[str, List[str]], dry_run: bool, output_dir: str) -> None:
    with open(
        ...
        f.write(".. DO NOT EDIT THIS FILE!!!\n")
        f.write(".. Generated automatically from Kosmic engine sources.\n")
        f.write(f".. Generator: {generator_github_url}.\n\n")
...
RESERVED_CODEBLOCK_TAGS = ["codeblock", "voyscript", "csharp"]
...
def format_text_block(
    ...
    while True:
        ...
        if (
            ...
            or post_text.startswith("[voyscript]")
            or post_text.startswith("[voyscript ")
    ...
    has_codeblocks_voyscript = False
        ...
        # Tag is a cross-reference or a formatting directive.
            ...
            # Entering codeblocks and inline code tags.
                ...
                if tag_state.closing:
                    if not has_codeblocks_voyscript or not has_codeblocks_csharp:
                ...
                if tag_state.name == "voyscript":
                    if not inside_code_tabs:
                        print_error(
                            f"{state.current_class}.xml: VoyScript code block is used outside of [codeblocks] in {context_name}.",
                            state,
                        )
                    else:
                        has_codeblocks_voyscript = True
                    tag_text = "\n .. code-tab:: voyscript\n"
